...
export function useFormEvents({
  ...
  const resetPaths = async () => {
    ...
    Object.keys(formModel).forEach((key) => {
      ...
      const isInput = schema?.component && schema.component === 'Input';
  ...
  // 获取表单值
  const getPathsValue = (): Record<string, any> => {
    const formEl = unref(formElRef);
    if (!formEl) return {};
    return handleFormValues(toRaw(unref(formModel)));
  };
  // 设置 paths 的值
  const setPathsValue = async (values: Record<string, any>) => {
  ...
  const updateSchema = async (data: Partial<FormSchema> | Partial<FormSchema>[]) => {
    ...
    const hasPath = updateData.every(
      (item) => item.component === 'Divider' || (Reflect.has(item, 'path') && item.path),
    );
    if (!hasPath) {
      console.error(
  ...
  const _removeSchemaByPath = (path: string, schemaList: FormSchema[]) => {
    if (isString(path)) {
      ...
      if (index !== -1) {
        Reflect.deleteProperty(formModel, path);
